...
import iotstar.vn.conn.ConnectionUtils;
import iotstar.vn.utils.MyUtils;
...
public class JDBCFilter implements Filter {
    ...
    private boolean needJDBC(HttpServletRequest request) {
        System.out.println("JDBC Filter");// Servlet Url-pattern: /spath/*=> /spath
        String servletPath = request.getServletPath(); //=> /abc/mnp
        String pathInfo = request.getPathInfo();
        String urlPattern = servletPath;
        if (pathInfo != null) {// => /spath/*
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (this.needJDBC(req)) {
            ...
            try {
                ...
                MyUtils.storeConnection(request, conn);// Cho phép request đi tiếp.
